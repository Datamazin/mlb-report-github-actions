---
name: sudo-lang
description: SudoLang v2.0 pseudolanguage specification for interacting with LLMs. Use this skill when interpreting, writing, or transpiling SudoLang programs. Covers interfaces, constraints, requirements, commands, pattern matching, pipe operators, and implicit LLM capabilities.
---

# SudoLang v2.0

> **Credit:** SudoLang was created by [Eric Elliott](https://github.com/paralleldrive). This specification is sourced from the [SudoLang repository](https://github.com/paralleldrive/sudolang/blob/main/sudolang.sudo.md).
...
- **Options** for customizing the behavior of your program.
...
> Note: The `cup` and `cap` operators are deprecated in favor of `union` and `intersection`.

### Commands

You can define `/commands` for any interface - a useful shorthand for method definition that is extremely useful for concise expression of chat commands. e.g. excerpt from the StudyBot program in the examples (shortcuts are optional, but very useful for frequently used commands):
...
You can also freely mix function syntax and command syntax. Function syntax is useful because it supports clearly delineated arguments, and call-time function modifiers.
...
Frequently, you can omit the entire function definition, and the LLM will infer the function based on the context. Use the `fn` or `function` keywords to document presence for users and LLMs:
...
The range operator `..` can be used to create a range of numbers:
...
Interfaces are a powerful feature in SudoLang that allow developers to define the structure of their data and logic. They're used to define the structure and behavior of objects, including constraints and requirements that must be satisfied. The `interface` keyword is optional.

## Requirements

Requirements enforce rules for interfaces and program behavior. They're great for input validation and rule enforcement.

Unlike constraints, requirements always throw errors when they're violated:
...
You can also `warn` instead of `require` to avoid throwing errors:
...
Constraints are a powerful feature that allow developers to guide the AI's output and behavior using declarative natural language. A constraint describes a condition that must always be satisfied, and the AI-inferred constraint solver continuously ensures that condition is met.
...
run(MinimumSalary) |> list(events) |> log:format=json |> wrapWith(code block)
```

## Mermaid Diagrams

SudoLang supports Mermaid diagrams to visualize architecture, flow control, and sequence descriptions:
...
The depth parameter controls the level of detail in the AI's response:

```SudoLang
Why is the sky blue? -depth 1        // simple, short answer
Why is the sky blue? -depth 10       // detailed answer
Why is the sky blue? -depth PhD      // expert-level answer
```

## Implicit LLM Capabilities

An LLM running SudoLang can:
...
- Lean into inference. Infer code and whole function bodies when you can.
- Limit code to the bare minimum required to clearly express flow control and composition.
...
Lint {
  ...
  * (code smells) => warn explain;
  ...
  default {
    ...
    offer tips to make code more understandable while adhering to the style guide.
    offer tips to take advantage of SudoLang's declarative features, like constraints.
  }
}
```
